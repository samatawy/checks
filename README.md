...
- [help/index.md](https://github.com/samatawy/checks/blob/main/help/index.md) for the docs overview
- [help/basic-checks.md](https://github.com/samatawy/checks/blob/main/help/basic-checks.md) for common validation patterns
- [help/composite-checks.md](https://github.com/samatawy/checks/blob/main/help/composite-checks.md) for `allOf(...)`, `anyOf(...)`, `oneOf(...)`, and `not(...)`
- [help/schema-check.md](https://github.com/samatawy/checks/blob/main/help/schema-check.md) for validating input from the supported JSON Schema subset
- [help/reading-results.md](https://github.com/samatawy/checks/blob/main/help/reading-results.md) for `flattened`, `nested`, and `validated` output
- [help/checks.md](https://github.com/samatawy/checks/blob/main/help/checks.md) for the full API reference
...
The most common patterns are `result({ language })` for the merged result tree, `result({ flattened: true })` for plain message arrays, and `result({ validated: 'partial' | 'strict' })` when you want a filtered clone of the validated input. For the complete result guide, see [help/reading-results.md](https://github.com/samatawy/checks/blob/main/help/reading-results.md).
...
That example shows the usual style: start at `ObjectCheck.for(...)`, branch to typed field validators, and use composition helpers only where you actually need alternatives. For a broader walkthrough, see [help/basic-checks.md](https://github.com/samatawy/checks/blob/main/help/basic-checks.md) and [help/composite-checks.md](https://github.com/samatawy/checks/blob/main/help/composite-checks.md).

If your validation rules already exist as a supported JSON schema document, see [help/schema-check.md](https://github.com/samatawy/checks/blob/main/help/schema-check.md) instead of translating everything to fluent rules by hand.

## Object Factory

If a class exposes static `validateInput(input)` and `fromValidInput(input)` methods, `ObjectFactory` can validate input and then hydrate an instance. See [help/object-factory.md](https://github.com/samatawy/checks/blob/main/help/object-factory.md).

## Coded Results

Stable result codes and translated output are supported, but optional. If you need catalogs, codes, or localized messages, see [help/coded-results.md](https://github.com/samatawy/checks/blob/main/help/coded-results.md).
...
For contributor workflows and publishing details, see [help/development.md](https://github.com/samatawy/checks/blob/main/help/development.md) and [help/publishing.md](https://github.com/samatawy/checks/blob/main/help/publishing.md).
